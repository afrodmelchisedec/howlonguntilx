...
export async function pingIndexNow(urls: string[]) {
  const key = process.env.INDEXNOW_KEY; // TODO: generate a key file, host it at https://howlonguntilx.com/<key>.txt, set this env var
  if (!key || urls.length === 0) return;
  try {
    await fetch('https://api.indexnow.org/indexnow', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ host: 'howlonguntilx.com', key, urlList: urls }),
    });
