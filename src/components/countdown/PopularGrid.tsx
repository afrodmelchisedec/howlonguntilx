    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sg">
        {events.map((ev,i)=>{
          ...
          const catClass = CAT_CLASS[ev.category] ?? 'gc-brand';
          return(
            <Link key={ev.slug} href={'/how-long-until-'+ev.slug}
              className={`ios-card interactive glow ${catClass} anim-fade-up block relative overflow-hidden p-4`}
              style={{ animationDelay:(i*55)+'ms' }}>
              <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: 'rgb(var(--glow))' }}/>
              <div className="text-caption mb-1">{ev.category}</div>
