...
export function ArticleCard({ toolSlug, slug, title, dek, heroImageUrl, glow }: { toolSlug: string; slug: string; title: string; dek: string; heroImageUrl?: string | null; glow: string }) {
  return (
    <Link href={`/tools/${toolSlug}/${slug}`} className="article-glow-card ios-card-nested press flex flex-col overflow-hidden anim-fade-up" style={{ border: `1px solid rgba(${glow}, 0.2)` }}>
      <img src={heroImageUrl || '/images/default-article-hero.jpg'} alt={title} className="w-full aspect-video object-cover" loading="lazy" />
      <div className="p-4">
