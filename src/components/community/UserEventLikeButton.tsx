// FILE: src/components/community/UserEventLikeButton.tsx
'use client';

import { useEffect, useState } from 'react';
import { useSession } from 'next-auth/react';

export function UserEventLikeButton({ userEventId, glow }: { userEventId: string; glow: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/user-events/${userEventId}/like`)
      .then(res => res.json())
      .then(data => {
        if (cancelled) return;
  ...
  async function handleClick() {
    ...
    try {
      const res = await fetch(`/api/user-events/${userEventId}/like`, { method: 'POST' });
