...
async function getAccessToken() {
  const auth = Buffer.from(`${process.env.PAYPAL_SANDBOX_CLIENT_ID}:${process.env.PAYPAL_SANDBOX_CLIENT_SECRET}`).toString('base64');
  const res = await fetch(`${PAYPAL_API_BASE}/v1/oauth2/token`, {
    method: 'POST',
    headers: { Authorization: `Basic ${auth}`, 'Content-Type': 'application/x-www-form-urlencoded' },
    body: 'grant_type=client_credentials',
  });
  if (!res.ok) throw new Error(`PayPal sandbox OAuth failed: ${res.status}`);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('PayPal sandbox webhook verification error:', err);
    return NextResponse.json({ error: 'Verification failed' }, { status: 400 });
  }
  if (!verified) {
    console.error('PayPal sandbox webhook signature invalid — rejecting.');
