// FILE: src/app/api/tools/pet-growth-gestation-tracker/route.ts
...
export async function GET() {
  ...
  const config = await prisma.petGrowthGestationConfig.findUnique({
...
export async function POST(req: NextRequest) {
  ...
  const { pets, notifyOnMilestone, shareLink } = body;
  if (!Array.isArray(pets)) {
    return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
  }

  try {
    const saved = await prisma.petGrowthGestationConfig.upsert({
      where: { userId: session.user.id },
      create: {
    ...
    console.error('Pet growth gestation config save error:', err);
